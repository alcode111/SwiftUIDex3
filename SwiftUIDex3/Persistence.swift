...
struct PersistenceController {
    ...
    init(inMemory: Bool = false) {
        container = NSPersistentContainer(name: "SwiftUIDex3")
        if inMemory {
            container.persistentStoreDescriptions.first!.url = URL(fileURLWithPath: "/dev/null")
        } else {
            container.persistentStoreDescriptions.first!.url = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.swiftuilp.Dex3Group")!.appending(path: "Dex3.sqlite")
        }
